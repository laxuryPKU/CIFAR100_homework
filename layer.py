...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        # parse additional args from init_lora_weights
        if isinstance(init_lora_weights, str) and "," in init_lora_weights:
            additional_args = init_lora_weights.split(",")[1:]
            init_lora_weights = init_lora_weights.split(",")[0]
            additional_args = {arg.split("=")[0]:arg.split("=")[1] if "=" in arg else True for arg in additional_args}
            if not hasattr(self, "additional_args"):
                self.additional_args = {}
            self.additional_args[adapter_name] = additional_args

        # change init_lora_weights to bool when init_lora_weights in ["true", "false"]
        if isinstance(init_lora_weights, str) and init_lora_weights.lower() == "true":
            init_lora_weights = True
        elif isinstance(init_lora_weights, str) and init_lora_weights.lower() == "false":
            init_lora_weights = False

        # different init_lora_weights method
    ...
    # A helper function to get additional_args parsed from init_lora_weights
    def get_additional_arg(self, adapter_name, arg_name, default=None):
        if not hasattr(self, "additional_args"):
            return default
        if adapter_name not in self.additional_args:
            return default
        if arg_name not in self.additional_args[adapter_name]:
            return default
        return self.additional_args[adapter_name][arg_name]

    def _apply_noised_lora(self, x, lora_A, lora_B, scaling, active_adapter):
        def add_noise(hidden_states):
            noise_type = self.get_additional_arg(active_adapter, "noise_type")
            # not specified
            if not noise_type:
                warnings.warn("set 'use_noised_lora' but there is no 'noise_type', will use default lora")
                noise = 0
            # uniform
            elif noise_type == "uniform":
                rank = hidden_states.shape[-1]
                mag_norm =  self.get_additional_arg(active_adapter, "noise_alpha", 1) / math.sqrt(rank)
                noise = torch.zeros_like(hidden_states).uniform_(-mag_norm, mag_norm)
            # normal
            elif noise_type == "gaussian":
                rank = hidden_states.shape[-1]
                gaussian_std =  self.get_additional_arg(active_adapter, "noise_alpha", 1) / rank
                noise = torch.zeros_like(hidden_states).normal_(0, gaussian_std)
            else:
                raise NotImplementedError
            return hidden_states + noise
        
        result = scaling * lora_B(add_noise(lora_A(x)))
        return result
    ...
    def pissa_init(self, adapter_name, init_lora_weights):
        ...
        # init_end = time.time()
        # print(f"{init_lora_weights} for matrix of shape {weight.shape} takes {init_end - init_start} seconds.")
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if self.get_additional_arg(active_adapter, "use_noised_lora", False):
                    x = dropout(x)
                    result = result + self._apply_noised_lora(x, lora_A, lora_B, scaling, active_adapter)
                else:
                    if not self.use_dora[active_adapter]:
